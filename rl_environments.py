...
class Vandelpol(gym.Env):
    def __init__(self, difficulty=1):
        ...
        self.observation_space = spaces.Box(
            low = np.array([-1.0*self.difficulty, -1.0*self.difficulty], dtype=jnp.float32),  
            high = np.array([1.0*self.difficulty, 1.0*self.difficulty], dtype=jnp.float32),
            dtype = jnp.float32)
        ...
        if self.difficulty == 1:
            ...
            self.init_spaces = [
                spaces.Box(
                low = np.array([-0.4, 0.75], dtype=jnp.float32),
                high = np.array([0.4, 1], dtype=jnp.float32),
                dtype=jnp.float32,
            )
            ]
            ...
            self.init_spaces = [
                spaces.Box(
                low = np.array([-2, 3], dtype=jnp.float32),
                high = np.array([2, 4], dtype=jnp.float32),
                dtype=jnp.float32,
            )
            ]
        ...
        self.target_spaces = [
            spaces.Box(
            low = np.array([-0.2, -0.2], dtype=jnp.float32), 
            high = np.array([0.2, 0.2], dtype=jnp.float32), 
            dtype = jnp.float32
        )
        ]
        self.unsafe_spaces = [
            spaces.Box(
            low = np.array([-1.0, -0.25], dtype=jnp.float32), 
            high = np.array([-0.8, 0.25], dtype=jnp.float32), 
            dtype=jnp.float32,
        ), 
            spaces.Box(
            low = np.array([0.8, -0.25], dtype=jnp.float32), 
            high = np.array([1, 0.25], dtype=jnp.float32), 
            dtype=jnp.float32,
        )
        ]
        
        # self.noise_coef = jnp.array([1, 1])        
        self.noise_coef = jnp.array([0.01, 0.02])        
        self.noise_bounds = jnp.array([[-1.0, -1.0], [1.0, 1.0]])
        if self.difficulty == 1:
            self.lipschitz_constant = 1.08
        elif self.difficulty == 2:
            self.lipschitz_constant = 1.34
        elif self.difficulty == 10:
            self.lipschitz_constant = 8.4
        else:
            self.lipschitz_constant = 100
        self.lipschitz_constant_normalize = 20.4

        self._max_steps = 200
        self._jax_rng = jax.random.PRNGKey(1)
        self.v_next = jax.vmap(self.next)
        self.v_add_noise = jax.vmap(self.add_noise)
        self.reset()
    ...
    def next(self, state):
        '''
        compute the next state without noise
        '''
        x = state[0]
        y = state[1]
        next_x = x - 0.2 * y
        next_y = y + 0.1 * (x + 0.5 * y * (x*x-1))
        return jnp.array([next_x, next_y])

    def add_noise(self, state, rng):
        '''
    ...
    def step(self):
        '''
        Here we assume the noise is additive, i.e., x_{t+1} = f(x_t) + noise. 
        f(x_t) is computed by next() function,
        and noise is generated by add_noise() function.
        '''
        self.steps += 1

        next_state = self.next(self.state)
        self._jax_rng, rng = jax.random.split(self._jax_rng)
        self.state = self.add_noise(next_state, rng)
    ...
    def integrate_noise(self, a: list, b: list):
        """
        Integrates noise over a list of cells.

        Parameters:
        a (list): Lower bounds for integration. 
            - a[0] is the lower bound for the first dimension
            - a[1] is the lower bound for the second dimension, etc.
        ...
        """
        dims = 2
        pmass = jnp.ones(a[0].shape[0])
        for i in range(dims):
            loc = self.noise_bounds[0][i]*self.noise_coef[i]
            scale = (self.noise_bounds[1][i] - self.noise_bounds[0][i])*self.noise_coef[i]
            ...
            # print(marginal_pmass)
        return pmass

class Poly1(gym.Env):
    # from Zhao2023TAC:Inner Approximating Robust Reach-Avoid Sets for Discrete-Time Polynomial Dynamical Systems
    def __init__(self):
        self.steps = None
        self.state = None
        self.has_render = False
        ...
        self.init_spaces = [
            spaces.Box(
            low = np.array([0, -0.6], dtype=jnp.float32),
            high = np.array([0.5, -0.4], dtype=jnp.float32),
            dtype=jnp.float32,
        )
        ]
        self.target_spaces = [
            spaces.Box(
            low = np.array([-0.2, 0.5], dtype=jnp.float32), 
            high = np.array([0.2, 0.7], dtype=jnp.float32), 
            dtype = jnp.float32
        )
        ]
        self.unsafe_spaces = [
            spaces.Box(
            low = np.array([-1.0, 0.0], dtype=jnp.float32), 
            high = np.array([-0.5, 0.5], dtype=jnp.float32), 
            dtype=jnp.float32,
        ), 
            spaces.Box(
            low = np.array([0.75, 0.0], dtype=jnp.float32), 
            high = np.array([1.0, 0.5], dtype=jnp.float32), 
            dtype=jnp.float32,
        )
        ]
    ...
    def next(self, state):
        ...
        x = state[0]
        y = state[1]
        next_x = x + 0.005*(-x-y+x*y)
        next_y = y + 0.01*(1-0.5*y)
        return jnp.array([next_x, next_y])
...
class NonPoly1(gym.Env):
    # manuelly construct
    def __init__(self, difficulty=1):
        self.steps = None
        self.state = None
        self.has_render = False
        self.name = f"NonPoly1"
        self.difficulty = difficulty
        # domain + init + target + unsafe + action
        self.action_space = None

        scale = np.minimum(self.difficulty, 10000)
        self.observation_space = spaces.Box(
            low = np.array([-1.0*self.difficulty, -1.0*self.difficulty], dtype=jnp.float32),  
            high = np.array([1.0*self.difficulty, 1.0*self.difficulty], dtype=jnp.float32),
            dtype = jnp.float32)
        ...
        self.init_spaces = [
            spaces.Box(
            low = np.array([-0.25*scale, 0.5*scale], dtype=jnp.float32),
            high = np.array([0.25*scale, 0.75*scale], dtype=jnp.float32),
            dtype=jnp.float32,
        )
        ]
        self.target_spaces = [
            spaces.Box(
            low = np.array([-0.2*scale, -0.2*scale], dtype=jnp.float32), 
            high = np.array([0.2*scale, 0.2*scale], dtype=jnp.float32), 
            dtype = jnp.float32
        )
        ]        
        self.unsafe_spaces = [
            spaces.Box(
            low = np.array([-0.25*scale, -0.75*scale], dtype=jnp.float32),
            high = np.array([0.25*scale, -0.5*scale], dtype=jnp.float32),
            dtype=jnp.float32,
        )
        ]
        self.noise_coef = jnp.array([0.02, 0.01])        
        self.noise_bounds = jnp.array([[-1.0, -1.0], [1.0, 1.0]])
        self.lipschitz_constant = 1.0
        self.lipschitz_constant_normalize = 2.5
    ...
    def next(self, state):
        ...
        x = state[0]
        y = state[1]
        next_x = x - 0.05*(0.25*x - 0.1*jnp.sin(x) + 0.1*y)
        next_y = y - 0.05*(0.2*x + 0.5*y)
        return jnp.array([next_x, next_y])
...
class NonPoly2(gym.Env):
    # manuelly construct
    def __init__(self, difficulty=1):
        self.steps = None
        self.state = None
        self.has_render = False
        self.name = f"NonPoly2"
        self.difficulty = difficulty
        # domain + init + target + unsafe + action
        self.action_space = None
        ...
        self.init_spaces = [
            spaces.Box(
            low = np.array([-0.5*scale, -0.25*scale], dtype=jnp.float32),
            high = np.array([-0.25*scale, 0.25*scale], dtype=jnp.float32),
            dtype=jnp.float32,
        )
        ]
        self.target_spaces = [
            spaces.Box(
            low = np.array([-0.5*scale, 0.5*scale], dtype=jnp.float32), 
            high = np.array([0.5*scale, 0.75*scale], dtype=jnp.float32), 
            dtype = jnp.float32
        )
        ]        
        self.unsafe_spaces = [
            spaces.Box(
            low = np.array([0.25*scale, -0.5*scale], dtype=jnp.float32),
            high = np.array([0.5*scale, -0.25*scale], dtype=jnp.float32),
            dtype=jnp.float32,
        )
        ]
        self.noise_coef = jnp.array([0.1, 0.1])        
        self.noise_bounds = jnp.array([[-1.0, -1.0], [1.0, 1.0]])
        if self.difficulty <= 1:
            self.lipschitz_constant = 1.5
        elif self.difficulty <=2:
            self.lipschitz_constant = 2.2
        elif self.difficulty <=10:
            self.lipschitz_constant = 3.9
        elif self.difficulty <=2:
            self.lipschitz_constant = 15.6
            
        self.lipschitz_constant_normalize = 5.7
    ...
    def next(self, state):
        ...
        y = state[1]
        next_x = x + 0.01*(x*x-y)
        next_y = y + 0.05*(5*jnp.log(1.0+x*x)*y)
        return jnp.array([next_x, next_y])
...
class LinearLQR(gym.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            low = np.array([-1.5, -1.5], dtype=jnp.float32),  
            high = np.array([1.5, 1.5], dtype=jnp.float32),
            dtype = jnp.float32)

        # self.observation_space = spaces.Box(
        #     low = np.array([-np.inf, -np.inf], dtype=jnp.float32),  
        #     high = np.array([np.inf, np.inf], dtype=jnp.float32),
        #     dtype = jnp.float32)
        ...
        self.init_spaces = [
            spaces.Box(
            low = np.array([-0.5, -0.5], dtype=jnp.float32),
            high = np.array([0.5, 0.5], dtype=jnp.float32),
            dtype=jnp.float32,
        )
        ]
        self.target_spaces = [
            spaces.Box(
            low = np.array([-0.1, -0.1], dtype=jnp.float32), 
            high = np.array([0.1, 0.1], dtype=jnp.float32), 
            dtype = jnp.float32
        )
        ]
        self.unsafe_spaces = [
            spaces.Box(
            low = np.array([-1.5, -0.5], dtype=jnp.float32), 
            high = np.array([-1.0, 0.5], dtype=jnp.float32), 
            dtype=jnp.float32,
        ), 
            spaces.Box(
            low = np.array([1.0, -0.5], dtype=jnp.float32), 
            high = np.array([1.5, 0.5], dtype=jnp.float32), 
            dtype=jnp.float32,
        )
        ]
...
#         Returns:
#         np.ndarray: The product of the marginal probabilities for each dimension.
#         """
#         dims = 2
#         pmass = np.ones(a[0].shape[0])
#         for i in range(dims):
